  ...
  useEffect(() => {
    const fetchTotalUsers = async () => {
      try {
        ...
        const annualSubscriptions = totalSubscription.filter(
          (sub) => sub.plan === "annual"
        ).length;
        const monthlySubscriptions = totalSubscription.filter(
    ...
    <div className="text-center p-10 bg-gray-100 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mt-10">
        ...
        <div className="bg-white rounded-lg shadow-lg p-5">
          ...
          <p className="text-3xl font-bold">
            <CountUp
              end={totalSubscriptions.annual + totalSubscriptions.monthly}
              duration={2.5}
            />
          </p>
        </div>
